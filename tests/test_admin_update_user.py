...
def test_admin_update_user_preserves_direct_invite_stage_when_form_omits_it(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            "/admin/user/update",
            data={"user_id": "7", "basket": "rook", "manual_points": "10"},
...
def test_admin_update_user_ignores_submitted_nickname(monkeypatch) -> None:
    target_user = _build_user(user_id=8, direct_invite_stage=None, direct_invite_group_number=None)

    async def fake_get(self, model, pk):
        return target_user if pk == 8 else None
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            "/admin/user/update",
            data={"user_id": "8", "nickname": "Hacked", "basket": "king"},
            follow_redirects=False,
        )

    assert response.status_code == 303
    assert response.headers["location"] == "/admin/users?msg=msg_status_ok"
    assert target_user.nickname == "User"
    assert target_user.basket == "king"
