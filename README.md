...
Обязательное требование Twitch для страницы `/watch`:
- домены в `TWITCH_PARENT_DOMAINS` должны **точно соответствовать** доменам, с которых пользователи открывают `/watch`;
- если используется несколько доменов, укажите их через запятую, например: `example.com,www.example.com`.

Если список parent-доменов не совпадает с фактическим доменом страницы, Twitch-плеер вернёт ошибку конфигурации и не загрузится.

### Короткий чек-лист деплоя
- сменился домен;
- обнови `TWITCH_PARENT_DOMAINS`;
- перезапусти сервис.
